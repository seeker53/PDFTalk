...
export default function DocumentClient({
  ...
  const transform: TransformToolbarSlot = (slot: ToolbarSlot) => ({
    ...
    CurrentPageLabel: slot.CurrentPageLabel,
    CurrentScale: slot.CurrentScale,
    GoToFirstPage: slot.GoToFirstPage,
    GoToLastPage: slot.GoToLastPage,
  ...
  const userProfilePic = userImage ? userImage : '/profile-icon.png';
      ...
      <div className="flex justify-between w-full lg:flex-row flex-col sm:space-y-20 lg:space-y-0 p-2">
        ...
        <div className="flex flex-col w-full justify-between align-center h-[90vh] no-scrollbar">
              ...
              {messages.map((message, index) => {
                ...
                return (
                  <div key={`chatMessage-${index}`}>
                    <div
                      className={`p-4 text-black animate ${message.role === 'assistant'
                        ? 'bg-gray-100'
                    ...
                    >
                      ...
                      {(isLastMessage || previousMessages) && sources && (
                        <div className="flex space-x-4 ml-14 mt-3">
                          {sources
                            .filter((source: any, idx: number, self: any[]) => {
                              const pageNumber = extractSourcePageNumber(source);
                              return self.findIndex((s: any) => extractSourcePageNumber(s) === pageNumber) === idx;
                            })
                            .map((source: any) => (
                              <button
                                ...
                                onClick={() =>
                                  pageNavigationPluginInstance.jumpToPage(Number(extractSourcePageNumber(source)) - 1)
                                }
                              >
          ...
          <div className="flex justify-center items-center sm:h-[15vh] h-[20vh]">
            <form onSubmit={(e) => handleSubmit(e)} className="relative w-full px-4 sm:pt-10 pt-2">
              <textarea
